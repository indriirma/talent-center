...
import { WarningAlert } from 'pages/component/PopupAlert';
...
const SignInModalForm = ({ signInOpen, signInClose, regOpen }) => {
  ...
  const handleWarnAlert = (title, descrip) => {
    setWarnTitle(title);
    setWarnDesc(descrip);
    setIsWarnOpen(true);
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      let message = '';
      if (title === "Password or email doesn't match!") {
        message = 'Please correct them and try logging in again';
      } else if (title.substring(0, 9) === 'Email isn') {
  ...
  return (
    signInOpen && (
      <>
        <WarningAlert open={isWarnOpen} close={() => setIsWarnOpen(false)} title={WarnTitle} description={WarnDesc} handleClick={() => {}} />
