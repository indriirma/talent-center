...
export const ColumnInfo = ({ title, content }) => {
  return (
    <>
      ...
      <Grid container sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, mb: '0.75rem', gap: '0.1rem', alignItems: 'center' }}>
        {content}
      </Grid>
    </>
  );
      ...
      {Array.isArray(medias) && medias.length > 0 ? (
        medias.map((media, index) => (
          <Box
            ...
            onClick={() => click(index)}
          >
            <MediaBox media={media} alter="photo" index={index} height={heightMedia} />
          </Box>
        ))
      ) : (
        <Box display="none" position="absolute" style={{ cursor: 'pointer' }} width="100%" height="100%">
          No media available
        </Box>
      )}
      <IconButton
...
export const MediaBox = ({ media, alter, index, height }) => {
  return (
    <>
      {media.type === 'img' ? (
        <img
