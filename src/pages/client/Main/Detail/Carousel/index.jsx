...
const DashIndexes = ({ activeIndex, medias }) => {
  return (
    <Box display="flex" justifyContent="center" alignItems="center" position="absolute" bottom="10px" left={0} right={0}>
      {medias.map((dash, index) => (
        <Box key={index} width="8px" height="8px" borderRadius="50%" backgroundColor={index === activeIndex ? '#2C8AD3' : '#DBDBDB'} mx="2px" />
      ))}
    </Box>
  );
};
...
export const Carousel = ({ medias, handleClick }) => {
  ...
  return (
    <CarouselComp
      ...
      otherComp={<DashIndexes medias={medias} activeIndex={activeIndex} />}
      height={200}
      width={300}
      heightMedia="600px"
