...
const Detail=()=>{ 
    ...
    const PositionTag =()=>{     
        return(
            <>
           { dataTalent.position!==undefined &&
            dataTalent.position.map((tag,index)=>(
                    <TagInfo key={index} tag={tag?.positionName}/>
                ))}
            </>
        )
    }
    const SkillSetTag =()=>{     
        return(
            <>
           { dataTalent.skillSet!==undefined &&
            dataTalent.skillSet.map((tag,index)=>(
                    <TagInfo key={index} tag={tag?.skillsetName}/>
                ))}
            </>
        )
    }  
                    ...
                    <Grid container sx={{display:'flex',flexDirection:{xs:'column',sm:'row'}}} >
                        <Grid item sx={{m:'1rem',display:'flex',flexDirection:'column',alignItems:'center'}}>
                            ...
                            <div style={{width:'100%'}}>
                                ...
                                <Button
                                    onClick={()=>{handleDownloadCV({talentId : dataTalent.talentId, talentName : dataTalent.talentName})}}
                        ...
                        <Grid item sx={{m:'1rem'}} >
                            <Grid>
                                ...
                                <SkillsetComp><ColumnInfo title={"Skill Set"} content={<SkillSetTag/>} /></SkillsetComp>
